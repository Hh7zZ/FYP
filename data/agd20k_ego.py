...
class TestData(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        # 路径结构: .../<aff_name>/<object>/<image>
        aff_name = p.parent.parent.name
        obj_name = p.parent.name

        image = self.load_img(str(p))
        AFF_CLASS = SEEN_AFF if self.divide == 'Seen' else UNSEEN_AFF

        gt_aff = AFF_CLASS.index(aff_name)

        mask_path = str(Path(self.mask_path) / aff_name / obj_name / (p.stem + ".png"))

        return image, gt_aff, obj_name, mask_path
