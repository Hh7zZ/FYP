...
def tokens_to_grid(attn_out, img_hw, patch):
    B, N_aff, L = attn_out.shape
    H, W = img_hw
    H_pad = (H + patch - 1) // patch * patch
    W_pad = (W + patch - 1) // patch * patch
    H_p, W_p = H_pad // patch, W_pad // patch
    if H_p * W_p != L:
        # 因子内拟合宽高比
        best = None
        ...
        for h in range(1, int(L ** 0.5) + 1):
            if L % h == 0:
                w = L // h
                for a, b in [(h, w), (w, h)]:
                    diff = abs(a / max(b, 1) - target_ratio)
        ...
        if best is not None:
            ...
            g = int(round(L ** 0.5))
            H_p = W_p = g
    return attn_out.reshape(B, N_aff, H_p, W_p)


def upsample_to_image(grid_maps, out_hw):
    H, W = out_hw
    return F.interpolate(grid_maps, size=(H, W), mode="bilinear", align_corners=False)